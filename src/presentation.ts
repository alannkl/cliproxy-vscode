...
import { DEFAULT_REFRESH_MS, ProviderQuota } from "./monitor";
import {
  Account,
  AccountQuota,
  providerNames,
...
export function isStale(
  ...
  return (
    Boolean(quota.error) ||
    (quota.summary !== undefined &&
      now - quota.summary.updatedAt > refreshIntervalMs * 2)
  );
}
...
function providerStatus(
  ...
  if (quota.provider === "claude") {
    const fiveHour =
      quota.summary?.seconds === 18000 ? quota.summary : undefined;
    const fable = quota.fableSummary;
    const units =
      fiveHour && fable
        ? Math.min(fiveHour.remainingUnits, fable.remainingUnits)
        : undefined;
  ...
  const color = remainingColor(
    quota.summary?.remainingUnits,
    isStale(quota, now, refreshIntervalMs),
  );
  const value = quota.summary
    ? `${remainingPercent(quota.summary.used)}%`
    : "-";
...
export function statusText(
  ...
  return providers
    .map((provider) =>
      providerStatus(
        state.find((p) => p.provider === provider) ?? {
          provider,
          accounts: [],
        },
        now,
...
function accountName(account: Account): string {
  return account.name
    .replace(/\.json$/i, "")
    .replace(/^(claude|codex)-[a-f0-9]{8,}-/i, "");
}
...
function accountWarning(
  ...
  if (
    ...
    now - entry.updatedAt > refreshIntervalMs * 2
  )
  ...
  if (quota.error && !quota.accounts.some((account) => account.error))
    return quota.error;
  return undefined;
}
...
function providerPreview(
  ...
  if (quota.summary)
    lines.push(
      `**${remainingPercent(quota.summary.used)}%** remaining · **${Math.round(quota.summary.remainingUnits)} / ${Math.round(quota.summary.totalUnits)} units** · ${windowLabel(quota.summary.seconds)}`,
      "",
    );
  if (quota.fableSummary)
    lines.push(
      `Fable weekly: **${remainingPercent(quota.fableSummary.used)}%** · **${Math.round(quota.fableSummary.remainingUnits)} / ${Math.round(quota.fableSummary.totalUnits)} units**`,
      "",
    );
  if (!quota.accounts.length)
    lines.push(markdown(quota.error ?? "No accounts found."), "");
  for (const entry of quota.accounts) {
    const warning = accountWarning(entry, quota, now, refreshIntervalMs);
    lines.push(
      `**${markdown(accountName(entry.account))}**${entry.account.planType ? " · " + markdown(planName(entry.account)!) : ""}${entry.account.disabled ? " · disabled" : ""}`,
      "",
    );
    ...
    for (const window of entry.windows) {
      ...
      const themeColor =
        color === "gray"
          ...
          : `--vscode-charts-${color}`;
      lines.push(
        `<tr><td>${html(windowName(window.label))}</td><td><span style="color:var(${themeColor});">${bar}</span> <strong>${remaining}%</strong></td><td>${resetIn(window.resetAt, now)}</td></tr>`,
      );
    }
    if (!entry.windows.length)
      lines.push("<tr><td>Quota unavailable</td></tr>");
...
export function quotaPreview(
  ...
  return [
    "[Open full view](command:cliproxyUsage.open)",
    "",
    ...state.map((quota) => providerPreview(quota, now, refreshIntervalMs)),
    "---",
  ...
  const sections = state
    .map((quota) => {
      const summary = quota.summary
        ? `${remainingPercent(quota.summary.used)}% · ${Math.round(quota.summary.remainingUnits)}/${Math.round(quota.summary.totalUnits)} units · ${windowLabel(quota.summary.seconds)}`
        : "-";
    ...
    .join("");
  const content = state.length
    ? sections
    : `<div class="setup"><p>${html(options.message)}</p>
