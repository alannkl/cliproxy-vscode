...
const options = {
  nonce: "test-nonce",
  message: "Configure a connection.",
  refreshing: false,
  checkedAt: now,
  now,
};
...
test("full view exposes provider and individual account refresh actions with safely encoded identities", () => {
  ...
  const targets = [
    ...page.matchAll(/href="command:cliproxyUsage.refreshTarget\?([^"]+)"/g),
  ].map((match) =>
    JSON.parse(decodeURIComponent(match[1]!.replaceAll("&#39;", "'"))),
  );
  assert.deepEqual(targets, [
    [{ provider: "claude" }],
    [{ provider: "claude", accountId: "c" }],
    [{ provider: "codex" }],
    [{ provider: "codex", accountId: "x" }],
...
test("Claude status shows five-hour and Fable percentages with the lower remaining-unit color", () => {
  ...
  const lowFable = {
    ...claude,
    fableSummary: { ...claude.fableSummary!, used: 90, remainingUnits: 10 },
  };
...
test("missing Claude windows show a dash rather than substituting weekly usage or zero", () => {
  assert.equal(
    statusText([{ ...claude, fableSummary: undefined }], now),
    "⚪ C: 98%/-  ⚪ X: -",
  );
  assert.equal(
    statusText(
      [{ ...claude, summary: { ...claude.summary!, seconds: 604800 } }],
      now,
    ),
    "⚪ C: -/94%  ⚪ X: -",
  );
  ...
  const partial: ProviderQuota = {
    ...
    accounts: [
      ...
      },
      {
        ...
        error: "Checks paused locally.",
        windows: codex.accounts[0]!.windows.map((w) => ({ ...w, used: 78 })),
      },
    ],
  };
...
test("discovery failures remain visible even when no accounts have been loaded", () => {
  const failed: ProviderQuota = {
    provider: "claude",
    accounts: [],
    error: "Cannot reach CLIProxy.",
  };
  for (const content of [
    quotaView([failed], options),
    quotaPreview([failed], now, now),
  ]) {
...
test("readings stay fresh for the configured interval instead of turning stale after two minutes", () => {
  ...
  assert.doesNotMatch(
    quotaPreview([claude], now, later, 600000),
    /Stale \/ unavailable/,
  );
  assert.doesNotMatch(
    quotaView([claude], { ...options, now: later, refreshIntervalMs: 600000 }),
    /Stale \/ unavailable/,
  );
  assert.match(
    quotaPreview([claude], now, later, 300000),
    /Stale \/ unavailable/,
  );
  assert.match(
    quotaView([claude], { ...options, now: later, refreshIntervalMs: 300000 }),
    /Stale \/ unavailable/,
  );
  assert.equal(
    statusText([{ ...claude, error: "Failed" }], now, 600000),
    "⚪ C: 98%/94%  ⚪ X: -",
  );
});
...
test("hover contains an Open full view link so it remains interactive when the pointer enters", () => {
  const preview = quotaPreview([claude, codex], now, now);
  assert.ok(
    preview.startsWith("[Open full view](command:cliproxyUsage.open)\n"),
  );
  assert.equal((preview.match(/\]\(command:/g) ?? []).length, 1);
});

test("hover bars use quota colors and stale bars stay gray", () => {
  const orange: ProviderQuota = {
    ...codex,
    accounts: codex.accounts.map((a) => ({
      ...a,
      windows: a.windows.map((w) => ({ ...w, used: 66 })),
    })),
  };
  assert.match(
    quotaPreview([claude], now, now),
    /<span style="color:var\(--vscode-charts-green\);">▰/,
  );
  assert.match(
    quotaPreview([orange], now, now),
    /<span style="color:var\(--vscode-charts-orange\);">▰/,
  );
  assert.match(
    quotaPreview([codex], now, now),
  ...
  accounts: [
    {
      account: {
        id: "c",
        name: "claude-1234abcd-example@example.com.json",
        provider: "claude",
        disabled: false,
      },
      updatedAt: now,
...
test("unconfigured sidebar offers Configure Connection and stale quota stays visibly stale", () => {
  assert.match(
    quotaView([], options),
    /href="command:cliproxyUsage.configure">Configure Connection<\/a>/,
  );
  const stale: ProviderQuota = {
    ...claude,
    error: "Some accounts could not be refreshed.",
    accounts: claude.accounts.map((a) => ({
      ...a,
      error: "Provider quota API returned HTTP 429.",
    })),
  };
  ...
  assert.match(
    page,
    /Stale \/ unavailable: Provider quota API returned HTTP 429/,
  );
...
test("account labels and errors cannot inject HTML or executable links into the sidebar or hover", () => {
  const malicious =
    "<script>alert(1)</script>[click](command:workbench.action.closeWindow)";
  const quota: ProviderQuota = {
    ...
    accounts: [
      {
        ...claude.accounts[0]!,
        account: { ...claude.accounts[0]!.account, name: malicious },
        error: malicious,
      },
    ],
  };
  ...
  assert.ok(
    preview.includes(
      "\\[click\\]\\(command:workbench\\.action\\.closeWindow\\)",
    ),
  );
...
test("hover preview shows all account windows with remaining bars and reset countdowns", () => {
  ...
  assert.match(
    preview,
    /5 hour<\/td><td><span[^>]+>▰{10}<\/span> <strong>98%<\/strong><\/td><td>2h 13m/,
  );
  assert.match(
    preview,
    /Weekly<\/td><td><span[^>]+>▰{10}<\/span> <strong>96%<\/strong><\/td><td>2d 2h/,
  );
  assert.equal((preview.match(/Last checked/g) ?? []).length, 1);
...
test("status uses one compact string with a large colored circle and a short dash when unavailable", () => {
  ...
  assert.equal(
    statusText([{ ...claude, error: "Failed" }, codex], now),
    "⚪ C: 98%/94%  🔴 X: 20%",
  );
});
