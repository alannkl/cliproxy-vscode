...
- The sidebar and each hover preview show one **Last checked** time after a refresh completes. Failed or stale readings remain labeled. Overall capacity totals exclude model-specific and code-review windows; Claude's Fable quota has its own separate total.
...
Windows in the same Cursor or VS Code profile share a cache and cross-process lock for each CPA connection. Automatic refreshes reuse a fresh snapshot. Concurrent manual refreshes share one fetch; another manual refresh after completion requests new data. Claude usage and profile lookups both run under this lock. Other windows pick up saved updates within about a second without calling the provider again.
